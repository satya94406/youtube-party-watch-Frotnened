...
// import { connectSocket, sendMessage } from "../services/socket"; // COMMENTED: backend socket
...
function Room() {
  ...
  const [participants, setParticipants] = useState({ [userId || "local"]: { username, role: "Admin", userId: userId || "local" } });
  ...
  const finalSuggestions = apiResults.length > 0 ? apiResults : localFiltered;
  ...
  return (
    <div className="min-h-screen bg-gray-900 text-white flex flex-col md:flex-row">
        ...
        <div className="flex flex-col flex-1 p-4">
          ...
          <div className="flex-1 overflow-y-auto space-y-2 pr-1">
            {messages.map((msg, idx) => {
              const isMe = msg.userId === (userId || "local");
              return (
                <div key={idx} className={`flex ${isMe ? "justify-end" : "justify-start"}`}>
                  <div className={`max-w-[75%] px-3 py-2 rounded-lg text-sm ${isMe ? "bg-blue-500 text-white" : "bg-gray-700 text-gray-200"}`}>
                    {!isMe && <div className="text-xs text-yellow-300 mb-1">{msg.username}</div>}
                    {msg.message}
                  </div>
                </div>
              );
          ...
          <div className="flex gap-2 mt-3">
            ...
            />
            <button onClick={handleSendMessage} className="bg-blue-500 hover:bg-blue-600 px-4 rounded-lg">
              Send
            </button>
          </div>
        </div>
      ...
      {inviteOpen && <InviteModal closeInviteModal={() => setInviteOpen(false)} roomId={roomId} />}
    </div>
  );
}
